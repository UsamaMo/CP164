...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        inserted = False
        crypt = hash(value)
        hash_slot = crypt % self._capacity
        if self._count == 0:
            self._table[hash_slot].append(value)
            inserted = True
        else:
            if self._table[hash_slot].is_empty():
                self._table[hash_slot].append(value)
                inserted = True
            else:
                i = self._table[hash_slot].index(value)
                if i == -1:
                    self._table[hash_slot].append(value)
                    inserted = True
        if inserted:
    ...
    def remove(self, key):
        ...
        if self._count == 0:
            ...
            if self._table[hash_slot].is_empty():
                ...
                i = self._table[hash_slot].index(key)
                if i == -1:
                    value = None
                else:
                    value = self._table[hash_slot][i]
                    self._table[hash_slot].pop(i)
                    self._count -= 1
        return value
    ...
    def _rehash(self):
        ...
        for slots in self._table:
            for i in slots:
                crypt = hash(i) % self._capacity
                new_table[crypt].append(i)
