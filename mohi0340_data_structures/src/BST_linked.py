...
class _BST_Node:
    ...
    def __init__(self, value):
        ...
        -------------------------------------------------------
        """
        # this value is the copy of the actual value
        self._value = deepcopy(value)
        # left node is none
        self._left = None
        # right node is none
        self._right = None
        # height is 1 as the first node
        self._height = 1
    ...
    def _update_height(self):
        ...
        if self._left is None:
            # height of left node equals 0
            left_height = 0
        # otherwise
        else:
            # height of left node is equal to height of left node in self
            left_height = self._left._height

        # if right node is none
        if self._right is None:
            # height of right node is equal to 0
            right_height = 0
        # otherwise
        else:
            # height of right node is equal to height of right node in self
            right_height = self._right._height

        # height of self binary tree is equal to the maximum between left node
        # and right node plus 1
...
class BST:
    ...
    def __init__(self):
        ...
        """
        # root = parent node
        # parent node equals none for starting
        self._root = None
        # count of BST is equal to 0 as there is nothing in BST
        # count equals to the number of nodes
        self._count = 0
    ...
    def is_empty(self):
        ...
        -------------------------------------------------------
        """
        # return True if BST is empty as Parent node is None
    ...
    def _insert_aux(self, node, value):
        ...
        if node is None:
            ...
            self._count += 1
            # inserted equals True to show that a new node has been inserted
            inserted = True
        # if the value of the node is less than the node value
        elif value < node._value:
            # General case: check the left subtree.
            # insert the node in the left
            node._left, inserted = self._insert_aux(node._left, value)
        # if the value of the node is greater than the node value
        elif value > node._value:
            ...
            # otherwise it means that nothing has been inserted
            inserted = False

        # also if something has been inserted update the height
        if inserted:
            # Update the node height if any of its children have been changed.
            node._update_height()

        # return the node and return true or false if inserted or not
    ...
    def retrieve(self, key):
        ...
        -------------------------------------------------------
        """
        # node equals root(parent) node
        node = self._root
        # value equal none
        value = None

        # while node(root node)is not empty and value is empty
        while node is not None and value is None:

            # if root value is greater than key
            if node._value > key:
                # node equals left node
                node = node._left
            # if root value is less than key
            elif node._value < key:
                # node equals right node
                node = node._right

            # if node value is equal to key
            elif node._value == key:
    ...
    def remove(self, key):
        ...
        -------------------------------------------------------
        """
        # understand this what does this mean?
        # return the first two values
        # self._root equals node
        # value equals value returned in remove auxilary function
        # using self remove aux (first node(self._root),key)
        self._root, value = self._remove_aux(self._root, key)

        # return only the matching key value from the node
        return value
    ...
    def height(self):
        ...
        height = self._height_aux(self._root)
    ...
    def _height_aux(self, node):
        ...
        if node is None:
            ...
            # if BST is not empty
            # the height of the BST is equal to maximum of (right node height , or , left node height) and add 1
            # add 1 as it is including the BST node
            height = max(self._node_height(node._left),
    ...
    def is_identical(self, other):
        ...
        -------------------------------------------------------
        """
        # check for the length of the self BST compared to length of other BST
        # if the lengths are not equal to each other, they ar enot identical
